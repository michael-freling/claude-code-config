...
func loadTemplatesForType(itemType ItemType) (*template.Template, []string, error) {
	dir := fmt.Sprintf("templates/%ss", itemType)
	...
	// First pass: parse _partials.tmpl if it exists
	partialsPath := filepath.Join(dir, "_partials.tmpl")
	partialsContent, err := fs.ReadFile(templatesFS, partialsPath)
	if err == nil {
		tmpl = template.New(string(itemType))
		tmpl, err = tmpl.Parse(string(partialsContent))
		if err != nil {
			return nil, nil, fmt.Errorf("failed to parse partials: %w", err)
		}
	} else {
		tmpl = template.New(string(itemType))
	}
